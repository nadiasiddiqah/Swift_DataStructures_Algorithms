...
func printEachLevel<T>(for tree: TreeNode<T>) {
    // 1. Initialize Queue data structure of TreeNode<T> type
    var queue = Queue<TreeNode<T>>()
    
    // 2. Keep track of nodes you'll need to work on before printing a new line
    var nodesLeftInCurrentlevel = 0
    
    // 3. Append tree nodes into queue
    while !queue.isEmpty {
        // Update nodesLeftInCurrentLevel based on queue.count
        ...
        while nodesLeftInCurrentlevel > 0 {
            ...
            // Add node's value to print statement
            ...
            // Subtract one from node
            nodesLeftInCurrentlevel -= 1
        }
        
        // If no nodes left in level, print all the level's node values
        print()
    }
}
